...
class BlogPostDAO:
    ...
    def add_comment(self, permalink, name, email, body):
        ...
        try:
            ...
            print "about to add the following comment: " + str(comment)

            result = self.posts.update_many({'permalink': permalink},
                            {'$push': {'comments': comment }})
                          # {'$push': {'comments': { 'author': 'debugAuthor', 'body': 'debugBody'}}}) THIS WORKS!!!
                          #{'$set': {'comments': { '$push': { comment }}}})
            num_matched = result.matched_count
            return num_matched  # Change this to return the number of documents updated by the code for HW 3.3
